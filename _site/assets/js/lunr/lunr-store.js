var store = [{
        ...
        "title": "[Network] 보안",
        "excerpt":"안전한 통신 기밀성 송신자와 지정된 수신자만이 전송되는 메시지의 내용을 이해할 수 있어야 한다. 메시지를 어떠한 방식으로든 암호화해야 한다. 메시지 무결성 전송중에 메시지의 내용이 변하지 않아야 한다. 종단점 인증 송신자와 수신자는 상대방이 실제로 누구인지 신원을 확인할 수 있어야 한다. 운영 보안 대부분의 기관은 공용 네트워크에 연결되어있기 때문에 공격에 노출될 수 있다....","categories": ["Network"],
        "tags": ["Netowrk"],
        "url": "/network/44/",
        "teaser": null
      },{
        ...
        "title": "[DB] 이상현상과 정규화",
        "excerpt":"이상현상 (Anomaly) 삽입이상 불필요한 데이터를 추가해야지만 삽입이 가능한 상황 갱신이상 같아야 하는 정보가 복수 개의 행에 표현될 경우, 데이터를 수정할 때 일부 정보만 갱신되고 일부는 변하지 않아 데이터 오류가 발생하는 현상 삭제이상 의도하지 않은 데이터까지 삭제해야만 지우고자 하는 데이터를 지울 수 있는 현상 → 이러한 이상 현상을 없애기 위해서 정규화를...","categories": ["데이터베이스"],
        "tags": ["정규화","DB"],
